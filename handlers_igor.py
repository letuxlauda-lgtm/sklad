...
def get_igor_menu():
    kb = [
        ...
        [KeyboardButton(text="війти з ролі")]
    ]
    return ReplyKeyboardMarkup(keyboard=kb, resize_keyboard=True)

# --- СОСТОЯНИЕ ДЛЯ Ігор ---
class IgorRole(StatesGroup):
    online = State()
...
# 🚪 ВХОД (igor3)
@router.message(F.text.lower() == "igor3")
async def igor_login(message: types.Message, state: FSMContext):
    try:
        db.init_igor_tables() 
        await state.set_state(IgorRole.online)
        await message.answer("👨‍🔧 Привіт, Ігор! Твоє меню:", reply_markup=get_igor_menu())
...
# =======================================================
@router.message(IgorRole.online, F.text == "⚙️звіт по роботі") 
async def send_report(message: types.Message):
    ...
    try:
        ...
        content = db.get_latest_igor_report()
...
# 📋 ЗАДАЧИ Ігора (Только IgorRole.online) - 
@router.message(IgorRole.online, F.text == "💳завдання,замовленя")
async def show_tasks(message: types.Message):
    await message.answer("Завантажую список завдань Ігора...")
    
    try:
        has_tasks = False

        # 1. ТЕРМІНОВІ ЗАВДАННЯ
        srochno = db.get_igor_tasks("srochno_callcentr")
        if srochno:
            has_tasks = True
        ...
        # 2. ПОТОЧНІ ЗАВДАННЯ
        zadaci = db.get_igor_tasks("zadaci_all")
        if zadaci:
            has_tasks = True
        ...
        # 3. ЗАМОВЛЕННЯ КАРТ
        karty = db.get_igor_tasks("kartu_all")
        if karty:
            has_tasks = True
        ...
        termin_tasks = db.get_termin_tasks("igor")
        if termin_tasks:
            has_tasks = True
...
# 🛒 СТОЛ ЗАКАЗОВ (IgorRole.online)
@router.message(IgorRole.online, F.text.lower() == "стіл замовлень")
...
async def process_item(message: types.Message, state: FSMContext):
    try:
        db.save_stol_zakazov("igor", message.text)
        await message.answer("✅ Замовлення додано в базу!", reply_markup=get_igor_menu())
        await state.set_state(IgorRole.online)
    except Exception as e:
...
# =======================================================
# 💸 ЗАТРАТЫ (IgorRole.online)
# =======================================================
@router.message(IgorRole.online, F.text.lower() == "витрати")
...
async def process_zatrata_sum(message: types.Message, state: FSMContext):
    try:
        ...
        db.save_zatrata("igor", name, amount)
        await message.answer(f"✅ Витрата '<b>{name}</b>' збережена!", parse_mode="HTML", reply_markup=get_igor_menu())
        await state.set_state(IgorRole.online)
    except ValueError:
...
async def process_task_done(callback: types.CallbackQuery):
    ...
    try:
        ...
        if db.close_task_in_db(table_name, task_id):
            ...
            await callback.answer("Завдання закрито!")
        else:
            await callback.answer("Помилка БД", show_alert=True)
    except Exception as e:
...
async def exit_igor(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("Ви вийшли з профілю Ігора.", reply_markup=ReplyKeyboardRemove())